...
class Utils:
    ...
    def compute_diffs(self, df, window_size=5, center=True):
        ...
        counties = df.pivot(
            index="date", columns="county_state", values="cases")
        for county in counties.columns:
            counties[county+self.diff_1_suffix] = counties[county].diff().rolling(window_size,
                                                                                  center=center).mean()
            counties[county+self.diff_2_suffix] = counties[county +
                                                           self.diff_1_suffix].diff()
        return counties
    ...
    def generate_plots(self, counties, state_names):
        for state in state_names:
            ...
            for county in county_names:
                df = counties[[county, county+self.diff_1_suffix, county+self.diff_2_suffix]
                              ].dropna()
                df['date'] = df.index.date
                if not df.empty:
                    df.plot.bar(x='date', subplots=True)
                    plt.savefig(
                        f'plots/{state}/{county}.png', bbox_inches='tight')
                    plt.close()
